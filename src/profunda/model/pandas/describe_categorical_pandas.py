...
from profunda.model.pandas.imbalance_pandas import column_imbalance_score
from profunda.model.pandas.utils_pandas import weighted_median
...
def length_summary_vc(vc: pd.Series) -> dict:
    ...
    summary = {
        ...
        "mean_length": (
            np.average(length_counts.index, weights=length_counts.values)
            if not length_counts.empty
            else np.nan
        ),
        "median_length": (
            weighted_median(length_counts.index.values, weights=length_counts.values)
            if not length_counts.empty
            else np.nan
        ),
