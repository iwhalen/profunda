    ...
    def loads(self, data: bytes) -> Union["ProfileReport", "SerializeReport"]:
        ...
        if (df_hash == self.df_hash) or (self.df is None):
            ...
            if (
                ...
                and loaded_description_set.package["profunda_version"] != __version__
            ):
                warnings.warn(
                    f"The package version specified in the loaded data is not equal to the version installed. "
                    f"Currently running on ydata-profiling {__version__} , while loaded data is generated by profunda, {loaded_description_set.package['profunda_version']}."
                )
